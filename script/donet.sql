...
DROP TABLE EVENT_ATTACHMENT;    --이벤트 첨부파일 == 안쓰는 테이블이에여 한번씩 드랍돌리고 이 코드지워주세요 !!!!
DROP TABLE VOLUNTEER;           
DROP TABLE VOLUNTEER_REPLY;     
DROP TABLE VOL_ATTACHMENT;      --선행활동 첨부파일 == 안쓰는 테이블이에여 한번씩 드랍돌리고 이 코드지워주세요 !!!!



DROP SEQUENCE SEQ_EV;
DROP SEQUENCE SEQ_EV_REPLY;
DROP SEQUENCE SEQ_EV_ATTA; -- 안쓰는 시퀀스에여 한번씩 드랍돌리고 이 코드지워주세요 !!!!
DROP SEQUENCE SEQ_VOL;
DROP SEQUENCE SEQ_VOL_REPLY;
DROP SEQUENCE SEQ_VOL_ATTA; -- 안쓰는 시퀀스에여 한번씩 드랍돌리고 이 코드지워주세요 !!!!
...
CREATE TABLE EVENT (
    ...
    EVENT_STATUS CHAR DEFAULT 'Y' CHECK(EVENT_STATUS IN ('Y','N'))
);
...
COMMENT ON COLUMN EVENT.EVENT_STATUS IS '이벤트 상태(Y/공개, N/비공개)';
...
CREATE TABLE GOODS(
    ...
    NUMBER_SUPPORTER NUMBER DEFAULT 0 NOT NULL,
...
CREATE TABLE REVIEW(
    ...
    RE_SU_TITLE VARCHAR2(500) NOT NULL,
    RE_CONTENT VARCHAR2(4000) NOT NULL,
    RE_COUNT NUMBER DEFAULT 0 NOT NULL,
    RE_DATE DATE DEFAULT SYSDATE NOT NULL,
    RE_IMG_ORIGINNAME VARCHAR2(4000) NOT NULL,
    RE_IMG_CHANGENAME VARCHAR2(4000) NOT NULL,
    RE_STATUS VARCHAR2(1) DEFAULT 'Y' CHECK(RE_STATUS IN ('Y','N')),
...
COMMENT ON COLUMN REVIEW.RE_SU_TITLE IS '후원한 종목 제목';
COMMENT ON COLUMN REVIEW.RE_TITLE IS '후기제목';
COMMENT ON COLUMN REVIEW.RE_CONTENT IS '후기내용';
COMMENT ON COLUMN REVIEW.RE_COUNT IS '조회수';
COMMENT ON COLUMN REVIEW.RE_DATE IS '작성일';
COMMENT ON COLUMN REVIEW.RE_IMG_ORIGINNAME IS '썸네일 변경전';
COMMENT ON COLUMN REVIEW.RE_IMG_CHANGENAME IS '썸네일 변경후';
COMMENT ON COLUMN REVIEW.RE_STATUS IS '후기상태';
...
CREATE TABLE REVIEW_IMAGE(   
    ...
    CONSTRAINT PK_RE_IMG_NO PRIMARY KEY(IMG_NO,RE_NO)
);

COMMENT ON COLUMN REVIEW_IMAGE.RE_NO IS '후원후기 번호';
COMMENT ON COLUMN REVIEW_IMAGE.IMG_NO IS '첨부파일 번호';
COMMENT ON COLUMN REVIEW_IMAGE.IMG_ORIGIN_NAME IS '이미지 기존 이름';
COMMENT ON COLUMN REVIEW_IMAGE.IMG_CHANGE_NAME IS '이미지 변경된 이름';
...
---------------------------------------
--후원결제번호 시퀀스
CREATE SEQUENCE SEQ_SUPPORT_PAYNO
INCREMENT BY 1
START WITH 1
CACHE 20;

--후원게시글 댓글 시퀀스
CREATE SEQUENCE SEQ_SUPPORT_COMMENT
INCREMENT BY 1
START WITH 1
CACHE 20;
