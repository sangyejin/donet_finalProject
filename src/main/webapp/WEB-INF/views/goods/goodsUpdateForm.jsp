...
<body>
	...
	<div class="main">
		<form id="updateForm" action="update" method="post" enctype="multipart/form-data"  autocomplete="off">
			<div class="div-content">
				<div class="div-goodsName">
					<label for="goodsName" style="display: block;" class="label-text">구호물품 이름</label> 
						<input type="text" placeholder="제목을 입력하세요" class="rounded-pill inputText"  id="goodsName" name="goodsName" value="${goods.goodsName }" required>
				</div>
				<div class="div-left col-lg-6 col-xs-12">
					<label for="thumbImg" class="label-text">대표 사진</label>
					<div class="div-thumbImg" id="thumbImg">
						<img id="img0"
							src="${pageContext.request.contextPath}/resources/upload_files/goods/${goods.thumbnailChangeName}">
					</div>
				</div>
				<div class="div-right col-lg-6 col-xs-12"
					style="padding-right: 0px;">
					<div class="div-input">
						<label for="cateogry" class="label-text">카테고리</label> <select
							name="goodsCategoryNo" id="category">
							<c:forEach var="category" items="${goodsCategoryList}">
								<c:if
									test="${goods.goodsCategoryNo eq category.goodsCategoryNo}">
									<option value="${category.goodsCategoryNo}" selected>${category.goodsCategoryName}</option>
								</c:if>
								<c:if
									test="${goods.goodsCategoryNo ne category.goodsCategoryNo}">
					...
					<div class="div-input">
						<label for="goodsPrice">가격</label> <input type="number"
							...
							class="rounded-pill inputText" id="goodsPrice" name="goodsPrice"
							value="${goods.goodsPrice }"> <span>원</span>
					</div>
					<div class="div-input">
						<label for="cateogry">후원처</label> <select name="addBeneficiary"
							id="addBeneficiary">
						...
						</select> <input id="btnInsertBeneficiary" class="btn btn-default"
							type="button" value="+">
						<div id="beneDiv">
							<table id="beneficiaryTable">
								...
								<tbody>
									<c:forEach var="bene" items="${oldBeneficiaryList}"
										varStatus="status">
										<tr>
											<td>${status.count}</td>
											<td><input class="notInput" name="beneficiaryNo"
												type="number" value="${bene.beNo}" style="border: none;"
												readonly></td>
											<td>${bene.beName}</td>
											<td><button class='btn btn-default btn-delete'
													data-action='delete' type="button">-</button></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
			...
			<div class="div-content col-xs-12" style="margin: auto;">
				<label for="content" class="label-text">내용</label>
				<textarea id="content" name="content" placeholder="">${goods.content }</textarea>
	...
	$("#btnInsertBeneficiary").click(function(){
		...
		const temp=`<tr>
						...
						<td><input class="notInput" name="beneficiaryNo" type="number" value="`+splitData[0]+`" style="border:none;" readonly></td>
						<td>`+splitData[1]+`</td>
						<td><button class='btn btn-default btn-delete' data-action='delete' type='button'>-</button></td>
					</tr>`;
	...
	function sendFile(file, el){
			...
			$.ajax({
				data : data,
				type : "POST",
				url : "${pageContext.request.contextPath}/goods/contentFile",
				cache : false,
				contentType : false,
				processData : false,
